...
![Project Config Access](/bublik-release/blog/img/pr-config-1.webp)
...
![Configurations](/bublik-release/blog/img/pr-config-2.webp)
...
![Project Form](/bublik-release/blog/img/projects-2.webp)

![Default attributes](/bublik-release/blog/img/pr-config-3.webp)
...
![Filter](/bublik-release/blog/img/projects-3.webp)
...
![Access](/bublik-release/blog/img/packet-capture-1.webp)

#### New Page

![Page](/bublik-release/blog/img/packet-capture-2.webp)

### Improved Shortcut Links For History And Run

#### Run

![Run](/bublik-release/blog/img/split-1.webp)

#### History

![History](/bublik-release/blog/img/split-2.webp)

### Report

#### Report Chart Toolbar

![Toolbar](/bublik-release/blog/img/report-1.webp)
...
6. Migrate configuration tags to meta: `python manage.py migrate_tags_config_to_meta`
7. Reformat configs: `python manage.py reformat_configs`
8. Stop nginx to avoid potential disruptions or conflicts during the migration process: `sudo systemctl stop nginx`
9. Assign the project based on the provided meta values (default `meta_name`: `PROJECT`) `python manage.py assign_project_by_meta -m <meta_name>`
...
10. Start nginx again: `sudo systemctl start nginx`
11. Create default configurations: `python manage.py initialize_configs`
12. `./scripts/deploy --steps django_settings run_services`
...
:::tip
After the update, the default dashboard appearance may differ from what you are used to, due to differences between project-specific and default configurations.  
To restore the default dashboard to its previous look, go to the configuration manager and adjust the dashboard settings (`DASHBOARD_DATE`, `DASHBOARD_HEADER`, `DASHBOARD_PAYLOAD`, `DASHBOARD_RUNS_SORT`, `DASHBOARD_DEFAULT_MODE`) in the default configuration to match your project configuration.
:::

:::tip
For readability and to avoid duplication, you can remove attributes from project configurations if their values match those in the corresponding default configurations.
:::
...
- **importruns:** fix missing project link on reimport ([95a8bf7](https://github.com/ts-factory/bublik/commit/95a8bf714a6131fcfeb8b75a8b60b3e9209bf816))
- **management:** fix meta validation on project assignment ([f3c1325](https://github.com/ts-factory/bublik/commit/f3c1325520430e4207c4df48d666235a981d0c02))
- **management:** fix initial run detection for project assignment ([91bb153](https://github.com/ts-factory/bublik/commit/91bb1530f35dc680b9403b0495e3fd1e33ef0728))
...
- **deploy:** add stop option for side services management ([918cb04](https://github.com/ts-factory/bublik/commit/918cb043ecdd0fb58bfde06c253972343f9c8018))
