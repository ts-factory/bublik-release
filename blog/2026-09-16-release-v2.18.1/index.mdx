...
AI chat is disabled by default. To enable it, follow these steps (full details
in [Setting Up AI Chat](/configuration/chat/setup)):

:::note
By default, AI chat is set up for **GPT (OpenAI)** and **Claude (Anthropic)**:
step 2 adds keys for both, step 4 generates an `ai` config referencing them. To
use a different provider or a custom gateway instead, add its key under a name
of your choice in step 2, then edit the config that step 4 generates to match.
:::

1. Enable AI chat in the configuration file `bublik/general.conf`:

   ```bash
   AI_CHAT_ENABLED="true"
   ```

2. Create `bublik/secrets.env` with your provider API key(s):
...
4. Apply updated Django settings, generate the default AI config, set up local
   file storage, and restart services:
...
5. Open the generated `ai` configuration in the configuration admin page under
   **No Project (Default)**, point each provider's `api_url` at the endpoint you
   actually use, delete the entries you don't run, save, and make sure the
   configuration is **active**. The generated config is a set of examples, not
   a working setup.

To have the assistant produce clickable links to runs and results, make sure
`BUBLIK_FQDN` is set in `bublik/general.conf` — it is empty by default on a
bare-metal installation, and without it the assistant returns answers without
links. See [Set the base URL for generated links](/configuration/chat/setup#3-set-the-base-url-for-generated-links).

#### Enabling S3 Storage

By default, chat-generated files go to local disk, which needs nothing
installed. To use S3-compatible object storage instead, choose one of the two
options below.

:::warning
Nothing migrates files between disk and object storage, so switch before
generating files you want to keep.
:::

**Option A — storage you already run**

1. Set the storage backend and S3 endpoint in `bublik/general.conf`:
...
2. Add S3 credentials to `bublik/secrets.env`:
...
3. Apply updated Django settings and restart services:

   ```bash
   ./scripts/deploy --steps django_settings run_services
   ```

Set `S3_PUBLIC_ENDPOINT_URL` as well only when the endpoint is reachable from
the user's browser. The bucket is created on the first upload.

**Option B — the bundled SeaweedFS**

1. Install the SeaweedFS binary at `$BUBLIK_HOME/seaweedfs/weed` (it is not
   packaged for Debian or Ubuntu):

   ```bash
   VERSION=4.40
   ARCH=linux_$(dpkg --print-architecture)   # linux_amd64 or linux_arm64
   curl -fsSLO https://github.com/seaweedfs/seaweedfs/releases/download/${VERSION}/${ARCH}.tar.gz
   mkdir -p ~/seaweedfs
...
2. Set the storage backend in `bublik/general.conf` — `S3_ENDPOINT_URL`
   already defaults to the bundled service:

   ```bash
   AI_CHAT_FILE_STORAGE_BACKEND="s3"
   ```

3. Create the SeaweedFS service, apply updated Django settings, and start it
   (`autostart_services` requires root rights):

   ```bash
   ./scripts/deploy --steps seaweedfs_service django_settings \
                            run_services autostart_services
   ```

SeaweedFS data lives in `$BUBLIK_HOME/seaweedfs/data`, which the database dump
does not cover — back it up alongside the database.

Full details for both options are in
[Store generated files in S3](/configuration/chat/setup#6-store-generated-files-in-s3-optional).
